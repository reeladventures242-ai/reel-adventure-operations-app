...
const seedData = {
  ...
  integrationStatus: { checkedAt: '', backendOnline: false, gmailConfigured: false, gmailConnected: false, gmailAccount: '', whatsappConfigured: false, whatsappWebhookConfigured: false, release: '' },
...
function migrateStore(existing = {}) {
  ...
  next.integrationStatus = { checkedAt: '', backendOnline: false, gmailConfigured: false, gmailConnected: false, gmailAccount: '', whatsappConfigured: false, whatsappWebhookConfigured: false, release: '', ...(next.integrationStatus || {}) };
...
async function refreshIntegrationStatus(options = {}) {
  try {
    ...
    const payload = await response.json();
    store.integrationStatus = { checkedAt: payload.updatedAt || new Date().toISOString(), backendOnline: true, gmailConfigured: Boolean(payload.gmail?.configured), gmailConnected: Boolean(payload.gmail?.connected), gmailAccount: payload.gmail?.email || '', whatsappConfigured: Boolean(payload.whatsapp?.configured), whatsappWebhookConfigured: Boolean(payload.whatsapp?.webhookConfigured), release: payload.release || '' };
    store.gmailOAuthStatus = store.integrationStatus.gmailConnected ? 'Connected' : store.integrationStatus.gmailConfigured ? 'Credentials Configured' : 'Not Connected';
