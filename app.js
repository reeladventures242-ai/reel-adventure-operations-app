...
const seedData = {
  ...
  whatsappInbox: [],
  whatsappDeliveryLog: [],
  whatsappAutomationLastRun: null,
  ...
  integrationStatus: { checkedAt: '', backendOnline: false, gmailConfigured: false, gmailConnected: false, gmailAccount: '', gmailRefreshable: false, gmailHasRefreshToken: false, gmailAccessTokenValid: false, gmailExpired: false, gmailNeedsReconnect: false, gmailStatus: '', gmailReason: '', whatsappConfigured: false, whatsappWebhookConfigured: false, release: '' },
...
const ownerOnlyIntegrationRoutes = new Set(['whatsapp', 'gmail-import']);
...
const PRODUCTION_MODE = typeof location === 'undefined' ? true : location.protocol !== 'file:';
...
function migrateStore(existing = {}) {
  ...
  next.whatsappQueue = (Array.isArray(next.whatsappQueue) ? next.whatsappQueue : []).map(normalizeWhatsAppQueueItem);
  next.whatsappInbox = Array.isArray(next.whatsappInbox) ? next.whatsappInbox : [];
  next.whatsappDeliveryLog = Array.isArray(next.whatsappDeliveryLog) ? next.whatsappDeliveryLog : [];
  next.whatsappAutomationLastRun = next.whatsappAutomationLastRun || null;
  ...
  next.notificationRules = normalizeNotificationRules(next.notificationRules);
  ...
  next.integrationStatus = { checkedAt: '', backendOnline: false, gmailConfigured: false, gmailConnected: false, gmailAccount: '', gmailRefreshable: false, gmailHasRefreshToken: false, gmailAccessTokenValid: false, gmailExpired: false, gmailNeedsReconnect: false, gmailStatus: '', gmailReason: '', whatsappConfigured: false, whatsappWebhookConfigured: false, release: '', ...(next.integrationStatus || {}) };
...
function weatherSummaryCards(trips = store.trips || []) {
  const counts = { Green: 0, Yellow: 0, Red: 0 }; currentWeatherTrips(trips).forEach((trip) => counts[tripWeatherRisk(trip)]++);
...
function renderWeatherAlertPanel(trips = store.trips || []) {
  const risks = currentWeatherTrips(trips).filter((trip) => ['Yellow','Red'].includes(tripWeatherRisk(trip)));
...
function generateWeatherAlerts() {
  currentWeatherTrips(store.trips || []).filter((trip) => ['Yellow','Red'].includes(tripWeatherRisk(trip))).forEach((trip) => {
...
function renderDashboardWeatherWidget() {
  const days = [...new Set((store.weatherRecords || []).map((record) => record.date).filter(isDateTodayOrFuture))].sort().slice(0, 7).map(dailyWeatherSummary).filter(Boolean);
...
async function pullSharedStore(options = {}) {
  ...
  try {
    ...
    sharedStoreSync.lastError = '';
    const remoteInboxChanged = payload.store && whatsappInboxSignature(payload.store.whatsappInbox) !== whatsappInboxSignature(store.whatsappInbox);
    if (payload.hasStore && payload.store && (options.initial || options.force || remoteInboxChanged || storeTime(payload.store.updatedAt) > storeTime(store.updatedAt))) {
...
function whatsappInboxSignature(items = []) {
  return (Array.isArray(items) ? items : []).slice(0, 25).map((item) => `${item.messageId || item.id || ''}:${item.receivedAt || ''}`).join('|');
}
...
function visibleNotifications(notices = store.notifications || []) {
  const current = notices.filter(isCurrentOperationalNotice);
  return canViewStockAlerts() ? current : current.filter((notice) => !isStockAlertNotice(notice));
}
...
function isDateTodayOrFuture(value) { return Boolean(value) && String(value) >= todayKey(); }
function isCurrentOrFutureTrip(trip = {}) { return !['Completed', 'Cancelled'].includes(trip.status) && (!trip.tripDate || isDateTodayOrFuture(trip.tripDate)); }
function isFutureWeatherTrip(trip = {}) { return !['Completed', 'Cancelled'].includes(trip.status) && isDateTodayOrFuture(trip.tripDate); }
function currentOperationalTrips(trips = store.trips || []) { return trips.filter(isCurrentOrFutureTrip); }
function currentWeatherTrips(trips = store.trips || []) { return trips.filter(isFutureWeatherTrip); }
function isCurrentOperationalNotice(notice = {}) {
  const tripId = notice.metadata?.tripId;
  if (!tripId || !['Weather', 'Assignment', 'Checklist'].includes(notice.category)) return true;
  const trip = (store.trips || []).find((item) => item.id === tripId);
  return !trip || isCurrentOrFutureTrip(trip);
}
...
async function init() {
  ...
  if (sharedStoreSync.enabled && typeof setInterval === 'function') setInterval(() => pullSharedStore(), 15000);
}
...
function renderRoleHomeDashboard() {
  ...
  const trips = currentOperationalTrips(visibleCalendarTrips()).sort(byDate);
  const today = new Date().toISOString().slice(0, 10);
...
function renderCrud(route) {
  ...
  if (route === 'trips') { renderAssignmentBoard(); page.querySelector('.page-stack')?.insertAdjacentHTML('beforeend', renderFuelTrackingOverview()); page.querySelector('.page-stack')?.insertAdjacentHTML('beforeend', `<div class="card card-pad trip-weather-overview"><div class="card-header"><h3>Trip Weather Risk</h3><button class="btn btn-outline btn-small" type="button" onclick="refreshLiveWeather()">Refresh Forecast</button></div>${weatherSummaryCards(currentWeatherTrips(visibleCalendarTrips()))}${currentWeatherTrips(visibleCalendarTrips()).sort(byDate).map((trip) => `<div class="weather-trip-row"><strong>${escapeHtml(formatDate(trip.tripDate))} · ${escapeHtml(trip.customer || 'Trip')}</strong>${weatherRiskBadge(trip)}</div>`).join('') || '<p class="empty-state">No scheduled trips from today forward.</p>'}</div>`); }
...
function saveRecord(event, route) {
  ...
  if (route === 'trips' && isCurrentOrFutureTrip(data)) {
...
function sortedDispatchTrips() {
  return currentOperationalTrips(store.trips || [])
...
function renderEmptyDispatchState() {
  return '<div class="empty-state">No current or upcoming trips need assignment. Create a trip with a vessel, captain, mate, date, and start time to populate the dispatch tree.</div>';
}
...
function generateChecklistReminders(now = new Date()) {
  currentOperationalTrips(store.trips || []).forEach((trip) => {
...
function renderOwnerDashboard() {
  ...
  const trips = currentOperationalTrips(store.trips.filter((trip) => ownerVessels.includes(trip.vessel))).sort(byDate);
  const notices = visibleNotifications(store.notifications || []).filter((notice) => selected === COMPANY_OWNER_NAME ? notice.recipientRole === 'Owner' || ownerVessels.includes(notice.metadata?.vessel) : (notice.recipientRole === VESSEL_OWNER_ROLE && notice.recipientName === selected) || ownerVessels.includes(notice.metadata?.vessel));
...
function integrationReadinessMarkup() {
  ...
  const gmailRefreshable = Boolean(status.gmailRefreshable);
  const gmailNeedsReconnect = Boolean(status.gmailNeedsReconnect);
  const gmailReason = status.gmailReason || '';
  const gmailStatusText = status.gmailStatus || (gmailConnected ? 'Connected' : gmailLive ? 'Ready to connect' : 'Needs credentials');
  const whatsappLive = Boolean(status.whatsappConfigured);
  const gmailWarning = gmailLive && (gmailNeedsReconnect || (gmailConnected && !gmailRefreshable)) ? `<p class="notice warning"><strong>Gmail token:</strong> ${escapeHtml(gmailReason || 'Refresh token missing. Reconnect Gmail so it stays connected after token expiry and redeploys.')}</p>` : '';
  return `<section class="legacy-tool settings-span integration-status-card"><div class="settings-card-head"><div><p class="eyebrow">Owner-only external sync</p><h3>WhatsApp & Gmail Sync</h3><p>Only the Owner profile can connect Gmail, sync email, or send through WhatsApp Business. Other profiles stay on the daily operations workflow.</p></div><button class="btn btn-outline btn-small" data-refresh-integration-status>Check Status</button></div><div class="integration-readiness-grid"><span><small>Backend</small><strong>${status.backendOnline ? 'Online' : 'Not checked'}</strong></span><span><small>Gmail OAuth</small><strong>${escapeHtml(gmailStatusText)}</strong>${integrationBadge(gmailLive && gmailConnected)}</span><span><small>Refresh token</small><strong>${gmailRefreshable ? 'Saved' : gmailConnected ? 'Missing' : 'Not connected'}</strong>${integrationBadge(gmailRefreshable)}</span><span><small>Gmail Account</small><strong>${escapeHtml(status.gmailAccount || 'None')}</strong></span><span><small>WhatsApp Business</small><strong>${whatsappLive ? 'Ready to send' : 'Manual send links'}</strong>${integrationBadge(whatsappLive)}</span><span><small>Webhook</small><strong>${status.whatsappWebhookConfigured ? 'Configured' : 'Needs verify token'}</strong>${integrationBadge(status.whatsappWebhookConfigured)}</span></div>${gmailWarning}<div class="legacy-actions"><button class="btn btn-primary" data-gmail-connect ${gmailLive ? '' : 'disabled'}>${gmailNeedsReconnect ? 'Reconnect Gmail' : 'Connect Gmail'}</button><button class="btn btn-outline" data-gmail-sync ${gmailConnected ? '' : 'disabled'}>Sync Gmail</button></div><div class="integration-checklist"><div><strong>Gmail flow</strong><p>Owner connects Google, then Sync Gmail pulls recent booking, invoice, quote, and confirmation emails into the existing review queue.</p></div><div><strong>WhatsApp flow</strong><p>Owner reviews queued messages, then can send through Meta WhatsApp Cloud API when credentials are configured. Webhook events are accepted at <code>/api/whatsapp/webhook</code>.</p></div></div></section>`;
}
...
async function refreshIntegrationStatus(options = {}) {
  try {
    ...
    const payload = await response.json();
    store.integrationStatus = { checkedAt: payload.updatedAt || new Date().toISOString(), backendOnline: true, gmailConfigured: Boolean(payload.gmail?.configured), gmailConnected: Boolean(payload.gmail?.connected), gmailAccount: payload.gmail?.email || '', gmailRefreshable: Boolean(payload.gmail?.refreshable), gmailHasRefreshToken: Boolean(payload.gmail?.hasRefreshToken), gmailAccessTokenValid: Boolean(payload.gmail?.accessTokenValid), gmailExpired: Boolean(payload.gmail?.expired), gmailNeedsReconnect: Boolean(payload.gmail?.needsReconnect), gmailStatus: payload.gmail?.status || '', gmailReason: payload.gmail?.reason || '', whatsappConfigured: Boolean(payload.whatsapp?.configured), whatsappWebhookConfigured: Boolean(payload.whatsapp?.webhookConfigured), release: payload.release || '' };
    store.gmailOAuthStatus = store.integrationStatus.gmailStatus || (store.integrationStatus.gmailConnected ? 'Connected' : store.integrationStatus.gmailConfigured ? 'Credentials Configured' : 'Not Connected');
    store.gmailConnectedAccount = store.integrationStatus.gmailAccount || store.gmailConnectedAccount || '';
...
async function syncOwnerGmail(mode = 'latest') {
  ...
  try {
    ...
    const message = error.message || 'Unknown Gmail sync error';
    if (/reconnect|refresh token|invalid_grant|expired/i.test(message)) {
      store.integrationStatus = { ...(store.integrationStatus || {}), gmailConnected: false, gmailNeedsReconnect: true, gmailRefreshable: false, gmailStatus: 'Reconnect required', gmailReason: message, checkedAt: new Date().toISOString() };
      store.gmailOAuthStatus = 'Reconnect required';
      saveStore();
      if (currentRoute === 'settings' || currentRoute === 'gmail') renderRoute(currentRoute);
    }
    toast(`Gmail sync failed: ${message}`);
  }
}
...
function normalizeNotificationRules(rules = {}) {
  return Object.fromEntries(Object.entries(DEFAULT_NOTIFICATION_RULES).map(([key, fallback]) => {
    const saved = rules && typeof rules === 'object' && !Array.isArray(rules) ? rules[key] || {} : {};
    return [key, { enabled: fallback.enabled, autoSendWhatsApp: fallback.autoSendWhatsApp, roles: [...fallback.roles], ...saved }];
  }));
}
function notificationRuleForCategory(category = '') {
  const value = String(category).toLowerCase();
  if (value.includes('checklist')) return 'checklists';
  if (value.includes('incident') || value.includes('weather') || value.includes('cruise')) return 'incidents';
  if (value.includes('payout') || value.includes('payroll') || value.includes('payment')) return 'payroll';
  if (value.includes('chat')) return 'chat';
  if (value.includes('customer') || value.includes('meeting') || value.includes('quote') || value.includes('invoice') || value.includes('trip reminder')) return 'customerMessages';
  if (value.includes('assignment')) return 'tripAssignments';
  return 'tripChanges';
}
function normalizeWhatsAppQueueItem(item = {}) {
  const recipientRole = item.recipientRole || 'Crew';
  const fallbackRule = DEFAULT_NOTIFICATION_RULES[notificationRuleForCategory(item.category)] || {};
  const customerApproval = recipientRole === 'Customer';
  return {
    ...item,
    id: item.id || makeId('wa-message'),
    recipientName: item.recipientName || recipientRole || 'Recipient',
    recipientRole,
    phoneNumber: item.phoneNumber || '',
    messageBody: item.messageBody || '',
    status: item.status || (whatsappDigits(item.phoneNumber) ? 'Ready' : 'Draft'),
    createdAt: item.createdAt || new Date().toISOString(),
    scheduledFor: item.scheduledFor || '',
    sentAt: item.sentAt || '',
    autoSend: Boolean(item.autoSend ?? (fallbackRule.autoSendWhatsApp && !customerApproval)),
    approvalRequired: Boolean(item.approvalRequired ?? customerApproval),
...
function whatsappEligibleForAutomation(item) { return item.status === 'Ready' && item.autoSend && !item.approvalRequired && whatsappDigits(item.phoneNumber) && (!item.scheduledFor || new Date(item.scheduledFor) <= new Date()); }
function whatsappStatusClass(status) { if (status === 'Failed') return 'red'; if (status === 'Sent Manually' || status === 'Sent via Business API') return 'green'; return status === 'Ready' ? 'blue' : 'gold'; }
function whatsappAutomationMarkup(){const queue=store.whatsappQueue||[],ready=queue.filter(item=>item.status==='Ready').length,eligible=queue.filter(whatsappEligibleForAutomation).length,approval=queue.filter(item=>item.approvalRequired&&item.status==='Ready').length,failed=queue.filter(item=>item.status==='Failed').length,last=store.whatsappAutomationLastRun,businessReady=Boolean(store.integrationStatus?.whatsappConfigured);return `<section class="card whatsapp-automation-panel"><div class="card-header"><div><p class="eyebrow">Notification rules engine</p><h3>WhatsApp Automation</h3></div><span class="badge ${businessReady?'green':'gold'}">${businessReady?'Business API ready':'Credentials needed'}</span></div><div class="integration-readiness-grid"><span><small>Ready queue</small><strong>${ready}</strong></span><span><small>Auto eligible</small><strong>${eligible}</strong></span><span><small>Needs approval</small><strong>${approval}</strong></span><span><small>Failed</small><strong>${failed}</strong></span><span><small>Last run</small><strong>${last?.at?new Date(last.at).toLocaleString():'Never'}</strong></span></div><div class="form-actions"><button class="btn btn-primary btn-small" data-whatsapp-process-queue ${businessReady&&eligible?'':'disabled'}>Process Ready Queue</button><button class="btn btn-outline btn-small" data-refresh-integration-status>Refresh Status</button></div>${notificationRulesMarkup()}${whatsappDeliveryLogMarkup()}</section>`;}
function notificationRulesMarkup(){store.notificationRules=normalizeNotificationRules(store.notificationRules);return `<details class="whatsapp-template-card"><summary><strong>Rules</strong><span class="chevron">⌄</span></summary><div class="gmail-readiness-grid">${Object.entries(DEFAULT_NOTIFICATION_RULES).map(([key,meta])=>{const rule=store.notificationRules[key];return `<span><small>${escapeHtml(meta.label)}</small><strong>${rule.enabled?'On':'Off'} · ${rule.autoSendWhatsApp?'Auto send':'Queue only'}</strong></span>`;}).join('')}</div></details>`;}
function whatsappDeliveryLogMarkup(){const logs=(store.whatsappDeliveryLog||[]).slice(0,5);return `<details class="whatsapp-template-card"><summary><strong>Delivery log</strong><span class="chevron">⌄</span></summary>${logs.length?logs.map(log=>`<div class="stat-row"><span>${escapeHtml(log.recipientName||log.category||'Message')}<small>${escapeHtml(log.detail||'')}</small></span><strong>${escapeHtml(log.status)}</strong></div>`).join(''):'<p class="empty-state">No Business API delivery attempts yet.</p>'}</details>`;}
function whatsappInboxMarkup(){const messages=(store.whatsappInbox||[]).slice(0,8);return `<section class="card table-card"><div class="card-header"><div><p class="eyebrow">Webhook inbox</p><h3>Incoming WhatsApp Messages</h3></div><div class="assignment-actions"><button class="btn btn-outline btn-small" type="button" onclick="pullSharedStore({force:true})">Refresh Inbox</button><span class="badge ${messages.length?'green':'gold'}">${messages.length} received</span></div></div><div class="whatsapp-queue-list">${messages.length?messages.map(item=>{const encoded=encodeURIComponent(String(item.text||''));return `<article class="whatsapp-queue-card"><div><span class="badge blue">${escapeHtml(item.status||'New')}</span><h4>${escapeHtml(item.profileName||item.from||'WhatsApp contact')}</h4><p>${escapeHtml(item.text||'No message text')}</p><small>${escapeHtml(item.from||'Unknown number')} · ${escapeHtml(new Date(item.receivedAt||Date.now()).toLocaleString())}</small></div><div class="whatsapp-queue-actions"><button class="btn btn-outline btn-small" onclick="copyWhatsAppText(decodeURIComponent('${encoded}'))">Copy</button></div></article>`;}).join(''):'<p class="empty-state">No incoming webhook messages recorded yet. Send a new WhatsApp test after deployment.</p>'}</div></section>`;}
async function processWhatsAppQueue(){if(!isCompanyOwnerUser())return toast('WhatsApp queue processing is owner-only.');try{toast('Processing ready WhatsApp queue...');const response=await fetch('api/whatsapp/process-queue',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({limit:10})});const payload=await response.json();if(!response.ok||payload.error)throw new Error(payload.error||`Queue process returned ${response.status}`);if(payload.store) store=normalizeStore(payload.store);addAudit('processed','WhatsApp Automation',`Processed ${payload.processed} message(s): ${payload.sent} sent, ${payload.failed} failed.`,{sent:payload.sent,failed:payload.failed});saveStore({localOnly:true});renderWhatsApp();toast(`WhatsApp queue processed: ${payload.sent} sent, ${payload.failed} failed.`);}catch(error){toast(`WhatsApp queue failed: ${error.message}`);}}
function whatsappQueueMarkup(){const businessReady=isCompanyOwnerUser()&&store.integrationStatus?.whatsappConfigured;return `<div class="card table-card"><div class="card-header"><div><p class="eyebrow">Shared queue</p><h3>WhatsApp Message Queue</h3></div><span class="badge blue">${store.whatsappQueue.length} messages</span></div><div class="whatsapp-queue-list">${store.whatsappQueue.length?store.whatsappQueue.map(item=>`<article class="whatsapp-queue-card"><div><span class="badge ${whatsappStatusClass(item.status)}">${escapeHtml(item.status)}</span><h4>${escapeHtml(item.recipientName)} · ${escapeHtml(item.recipientRole)}</h4><p>${escapeHtml(item.category)} · ${escapeHtml(item.phoneNumber || 'Phone missing')}</p><small>${escapeHtml(new Date(item.createdAt).toLocaleString())} · ${escapeHtml(item.createdBy)}</small>${item.scheduledFor?`<small>Scheduled: ${escapeHtml(new Date(item.scheduledFor).toLocaleString())}</small>`:''}<small>${item.autoSend?'Auto send':'Manual'} · ${item.approvalRequired?'Approval required':'No approval required'} · Attempts: ${Number(item.attemptCount||0)}</small>${item.lastError?`<small>${escapeHtml(item.lastError)}</small>`:''}${item.businessMessageId?`<small>Business ID: ${escapeHtml(item.businessMessageId)}</small>`:''}</div><div class="whatsapp-queue-actions">${businessReady?`<button class="btn btn-primary btn-small" data-whatsapp-business-send="${item.id}">Send via Business API</button>`:`<button class="btn btn-primary btn-small" onclick="openWhatsAppMessage('${item.id}')">Open in WhatsApp</button>`}<button class="btn btn-outline btn-small" onclick="copyWhatsAppText(store.whatsappQueue.find(x=>x.id==='${item.id}').messageBody)">Copy</button><select aria-label="Message status" onchange="updateWhatsAppStatus('${item.id}',this.value)">${WHATSAPP_STATUSES.map(s=>`<option ${s===item.status?'selected':''}>${s}</option>`).join('')}</select></div></article>`).join(''):'<p class="empty-state">No WhatsApp drafts yet.</p>'}</div></div>`;}
...
const GMAIL_ACTION_OPTIONS = ['Create Booking','Create Trip','Create Invoice / Quote','Add to Calendar','Save as Draft','Ignore','Create WhatsApp confirmation draft','Create payment reminder draft','Create meeting point message draft'];
const GMAIL_AI_AUTO_CONFIDENCE = 70;
...
function gmailImportAiDecision(item = {}) {
  const text = `${item.subject || ''}\n${item.rawMessagePreview || ''}`.toLowerCase();
  const missing = missingGmailImportFields(item);
  const completeTrip = ['customerName','tourDate','startTime','guestCount','tourType'].every((key) => item[key]);
  const hasPrice = Boolean(item.totalPrice || item.deposit || item.balance);
  const hasBookingSignal = /book(?:ing|ed)|reservation|confirmed|confirmation|order|itinerary|voucher|tour date|departure|guest|passenger/.test(text) || Boolean(item.bookingReference);
  const hasInquirySignal = /inquiry|interested|available|availability|quote|price|how much|would like|can we book|question/.test(text);
  const hasPaymentSignal = /payment|paid|deposit|balance|invoice|receipt|refund|card|remaining/.test(text) || Boolean(item.balance || item.deposit);
  const hasMeetingSignal = /meeting point|pickup|pick up|where do we meet|location|directions|dock|terminal|marina/.test(text);
  let action = 'Save as Draft';
  let category = 'Needs Review';
  let reason = 'Not enough context to safely create an operations record.';
  if (hasPaymentSignal && !completeTrip && hasPrice) {
    action = 'Create payment reminder draft';
    category = 'Payment';
    reason = 'Payment language or balance details were detected, but trip details are incomplete.';
  } else if (hasMeetingSignal && !completeTrip) {
    action = 'Create meeting point message draft';
    category = 'Meeting Point';
    reason = 'Meeting or pickup language was detected without enough trip data for calendar creation.';
  } else if (completeTrip && hasBookingSignal) {
    action = 'Add to Calendar';
    category = 'Confirmed Booking';
    reason = 'Customer, date, time, guest count, tour type, and booking context were detected.';
  } else if (completeTrip) {
    action = 'Create Trip';
    category = 'Trip Request';
    reason = 'Core trip fields are present, but confirmation language is weaker.';
  } else if (hasInquirySignal) {
    action = 'Save as Draft';
    category = 'Inquiry';
    reason = 'Inquiry language was detected; missing trip details should be reviewed before calendar creation.';
  } else if (hasPaymentSignal) {
    action = 'Create Invoice / Quote';
    category = 'Payment / Quote';
    reason = 'Pricing or invoice language was detected.';
  }
  const confidence = Math.max(Number(item.confidenceScore || 0), completeTrip && hasBookingSignal ? 88 : 0, completeTrip ? 76 : 0);
  const autoReady = confidence >= GMAIL_AI_AUTO_CONFIDENCE && missing.length === 0 && !gmailDuplicateMatches(item).length && ['Add to Calendar','Create Trip'].includes(action);
  return { action, category, reason, confidence, autoReady };
}

function enrichGmailImportAi(item) {
  const decision = gmailImportAiDecision(item);
  item.aiCategory = decision.category;
  item.recommendedAction = decision.action;
  item.aiDecisionReason = decision.reason;
  item.aiConfidenceScore = decision.confidence;
  item.autoActionReady = decision.autoReady;
  return item;
}
...
function queueTripWhatsAppDrafts(trip, source = {}) {
  ...
  if (!isCurrentOrFutureTrip(trip)) return 0;
  let count = 0;
...
function saveGmailImportReview(event, id) { event.preventDefault(); const item = store.gmailImports.find((record) => record.id === id); if (!item) return; const data = Object.fromEntries(new FormData(event.target).entries()); let action = data.actionOverride || data.action || item.recommendedAction || 'Save as Draft'; delete data.action; delete data.actionOverride; const duplicateDecision = data.duplicateDecision || ''; delete data.duplicateDecision; Object.assign(item,data); enrichGmailImportAi(item); action = action === 'Use AI recommendation' ? item.recommendedAction : action; const duplicates = gmailDuplicateMatches(item); if (duplicates.length && !duplicateDecision) return toast('Possible Duplicate Booking Found. Choose Update Existing, Create New, or Cancel.'); if (duplicateDecision === 'Cancel') return toast('Import action cancelled.'); item.possibleDuplicates = duplicates.map(({id,type,matchCount}) => ({id,type,matchCount})); item.reviewedAt = new Date().toISOString(); item.reviewedBy = currentUserLabel(); item.importStatus = 'Reviewed'; const mapped = gmailImportToUploadData(item); linkGmailImportCustomer(item);
  ...
  addAudit('reviewed','Gmail Import',`${action} completed from AI-assisted Gmail review.`,{emailId:item.emailId,source:item.source,customerProfileId:item.customerProfileId,aiCategory:item.aiCategory,aiConfidenceScore:item.aiConfidenceScore}); addNotification('Gmail import action completed',`${action} completed for ${item.customerName || item.subject || 'imported email'}.`,'success',{category:'Gmail Import',emailId:item.emailId}); saveStore(); renderGmailImport(); toast(`${action} completed from AI-assisted review.`); }
...
function gmailImportSettingsMarkup() {
  const settings = store.gmailImportSettings;
  const integration = store.integrationStatus || {};
  const gmailConfigured = Boolean(integration.gmailConfigured);
  const gmailConnected = Boolean(integration.gmailConnected);
  const gmailRefreshable = Boolean(integration.gmailRefreshable);
  const gmailNeedsReconnect = Boolean(integration.gmailNeedsReconnect);
  const gmailReason = integration.gmailReason || '';
  const eyebrow = gmailConnected ? 'Live Gmail connected - owner sync enabled' : gmailConfigured ? 'Gmail credentials ready - connect owner account' : 'Manual Gmail import - credentials needed';
  const noticeClass = gmailConnected && gmailRefreshable ? 'success' : 'warning';
  const noticeText = gmailConnected && gmailRefreshable ? 'Live Gmail sync is connected and has a saved refresh token, so it can refresh automatically after token expiry and redeploys.' : gmailNeedsReconnect ? (gmailReason || 'Reconnect Gmail. The saved access token is expired and no refresh token is available.') : gmailConfigured ? 'Gmail API credentials are configured in Render. Connect or reconnect the owner Gmail account so Google returns a refresh token.' : 'No live Gmail API: Gmail credentials are not configured yet. Manual paste/upload import remains available until Render has Gmail OAuth credentials.';
  return `<details class="legacy-tool settings-span gmail-settings" open><summary><div><p class="eyebrow">${eyebrow}</p><h3>Gmail Import Settings</h3></div><span class="chevron">⌄</span></summary><form onsubmit="saveGmailImportSettings(event)"><div class="gmail-readiness-grid"><span><small>gmailOAuthStatus</small><strong>${escapeHtml(store.gmailOAuthStatus)}</strong></span><span><small>gmailConnectedAccount</small><strong>${escapeHtml(store.gmailConnectedAccount || integration.gmailAccount || 'None')}</strong></span><span><small>refreshToken</small><strong>${gmailRefreshable ? 'Saved' : gmailConnected ? 'Missing' : 'Not connected'}</strong></span><span><small>lastSyncTime</small><strong>${escapeHtml(store.lastSyncTime || 'Never')}</strong></span><span><small>syncCursor</small><strong>${escapeHtml(store.syncCursor || 'Not set')}</strong></span><span><small>importRules</small><strong>${store.importRules.length} placeholders</strong></span><span><small>sourceRules</small><strong>${store.sourceRules.length} placeholders</strong></span></div><div class="form-grid"><label class="toggle-row"><input name="enabled" type="checkbox" ${settings.enabled ? 'checked' : ''}> Enable Gmail Import framework</label><label>Default Import Action<select name="defaultImportAction">${['Save as Draft','Create Booking','Create Trip','Create Invoice / Quote','Add to Calendar'].map((value) => `<option ${settings.defaultImportAction === value ? 'selected' : ''}>${value}</option>`).join('')}</select></label><label>Duplicate Detection Strictness<select name="duplicateDetectionStrictness">${['Strict','Standard','Relaxed'].map((value) => `<option ${settings.duplicateDetectionStrictness === value ? 'selected' : ''}>${value}</option>`).join('')}</select></label><label class="toggle-row"><input type="checkbox" checked disabled> Review Required Before Save (always on)</label></div><fieldset><legend>Allowed Sources</legend><div class="gmail-source-options">${GMAIL_IMPORT_SOURCES.map((source) => `<label><input type="checkbox" name="allowedSources" value="${escapeHtml(source)}" ${settings.allowedSources.includes(source) ? 'checked' : ''}> ${escapeHtml(source)}</label>`).join('')}</div></fieldset><p class="notice ${noticeClass}"><strong>Gmail status:</strong> ${escapeHtml(noticeText)}</p><button class="btn btn-primary">Save Gmail Import Settings</button></form></details>`;
}
...
function assistantScopedTrips() { return currentOperationalTrips(visibleCalendarTrips()); }
